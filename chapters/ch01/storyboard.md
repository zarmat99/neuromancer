# Capitolo 1 - storyboard editoriale — v0.2

Lo storyboard definisce ritmo e geometria. Le lettere indicano le vignette in
ordine di lettura occidentale, da sinistra a destra e dall'alto in basso.

## Tavola 1 - Il colore di un canale morto

- **Layout:** A panoramica al vivo (40%); B-C-D fascia centrale; E-F fascia bassa.
- **A:** porto e cielo, orizzonte schiacciato, titolo piccolo nello spazio negativo.
- **B:** immersione nella folla di Ninsei.
- **C:** spalla di Case che apre la porta del Chat.
- **D:** primo piano di Ratz dietro al bancone.
- **E:** dettaglio protesi rosa che riempie una birra.
- **F:** two-shot Case/Ratz; la battuta su Wage chiude a destra.
- **Ritmo:** discesa dal mondo alla minaccia personale.

## Tavola 2 - La caduta

- **Layout:** A-B stretti; C centrale orizzontale; D-E verticali in basso.
- **A:** Case reagisce con sarcasmo.
- **B:** silenzio del bar e volto irrigidito.
- **C:** matrice, unica vignetta con gutter nero.
- **D:** letto dell'hotel di Memphis, cinghie e allucinazione.
- **E:** Case rannicchiato nella capsula presente.
- **Ritmo:** il passato irrompe e richiude Case nel corpo.

## Tavola 3 - Night City

- **Layout:** A panoramica; B-C; D-E-F.
- **A:** Case piccolo sotto le insegne di Ninsei.
- **B:** capsule impilate vicino al porto.
- **C:** mano con pillola rosa e espresso.
- **D:** specchi rossi del Jarre.
- **E:** dettagli iperfocalizzati del tavolo graffiato.
- **F:** Linda compare moltiplicata negli specchi.
- **Ritmo:** accelerazione sensoriale, apparizione quieta.

## Tavola 4 - Linda

- **Layout:** A grande ricordo; B-C-D montaggio; E-F presente.
- **A:** Linda alla console dell'arcade, volto a bande di luce.
- **B:** pioggia su Harajuku e mani unite.
- **C:** capsula condivisa, non esplicita.
- **D:** dipendenza, mano tremante vicino a una sigaretta.
- **E:** Linda al tavolo del Jarre.
- **F:** close-up occhi grigi: avvertimento su Wage.

## Tavola 5 - Cinquanta

- **Layout:** cinque vignette regolari, ultima larga.
- **A:** Case finge indifferenza.
- **B:** Linda piegata dal tremore.
- **C:** banconota passata sotto il tavolo.
- **D:** mano di Linda chiude la tasca.
- **E:** Case fuori dalla porta; occhi di Linda riflessi nel reticolo rosso.

## Tavola 6 - Stelle di cromo

- **Layout:** A-B; C grande verticale; D-E-F.
- **A:** folla di Ninsei letta come flusso dati.
- **B:** tatuaggio aziendale sulla mano di un impiegato.
- **C:** shuriken cromati su velluto rosso, riflesso di Case.
- **D:** anticamera di Deane.
- **E:** Deane dietro la scrivania.
- **F:** Case lascia l'ufficio, porta che si richiude.

## Tavola 7 - La coda

- **Layout:** A panoramica stretta; B-C; D; E dominante.
- **A:** riflesso nella boutique chirurgica.
- **B:** sagoma scura e argento sugli occhi.
- **C:** Case scatta nella folla.
- **D:** Shin nega una pistola immediata.
- **E:** dimostrazione del cobra telescopico.

## Tavola 8 - Campo di dati

- **Layout:** A-B-C fascia alta; D centrale; E al vivo in basso.
- **A:** Case nasconde il cobra.
- **B:** Zone al Chat, pupille enormi.
- **C:** domanda sulla donna dai capelli scuri.
- **D:** riflesso di Molly nella folla.
- **E:** Case corre verso l'arcade, Ninsei trasformata in vettori di informazione.

## Tavola 9 - La trappola

- **Layout:** sei pannelli progressivamente più stretti.
- **A:** esplosione olografica nell'arcade.
- **B:** Case sale le scale.
- **C:** ordina alla receptionist di chiamare la sicurezza.
- **D:** sfonda la prima porta.
- **E:** forza il cubicolo adiacente e rompe la finestra.
- **F:** buio, cobra esteso, punta che pulsa.

## Tavola 10 - Paura reale

- **Layout:** A stretta al vivo; B-C; D grande diagonale; E-F in basso.
- **A:** stivali che passano davanti alla porta.
- **B:** occhio di Case, bravado dissolto.
- **C:** cobra richiuso nella mano.
- **D:** salto dalla finestra, composizione diagonale.
- **E:** Case tra rottami, caviglia ferita.
- **F:** testa di Molly nel riquadro, lenti d'argento, poi assenza.

## Tavola 11 - Numero 92

- **Layout:** A-B; C panoramica verticale; D-E.
- **A:** .22 con guance rosse consegnata da Shin.
- **B:** seconda pillola inghiottita a secco.
- **C:** cortile del Cheap Hotel, capsule impilate.
- **D:** interno claustrofobico della 92.
- **E:** computer, ghiacciaia, fiasca e pistola; telefono muto.

## Tavola 12 - Wage

- **Layout:** A-B-C; D grande; E stretta finale.
- **A:** chiamata a Hong Kong senza risposta.
- **B:** Snake Man interrompe la chiamata.
- **C:** Case guarda la pistola.
- **D:** Chat quasi vuoto; Ratz al tavolo e Kurt sul bancone.
- **E:** Wage e i due Joeboy riempiono la porta.

## Tavola 13 - Debito

- **Layout:** A grande; B-C-D; E-F.
- **A:** .22 puntata, Ratz afferra l'arma.
- **B:** caricatore sul tavolo.
- **C:** Wage nega, sincera irritazione.
- **D:** fiasca sparisce nella giacca di Wage.
- **E:** alba grigio-rosa su Ninsei; inserto successivo della .22 restituita a
  Shin con le cartucce. Case esce disarmato.
- **F:** custode del Cheap Hotel conferma la visita di Linda.

## Tavola 14 - Molly

- **Pianta:** `LOC_COFFIN_92_V2`, 3,00 × 1,00 × 1,45 m. Asse unico dal portello
  alla testata di fondo.
- **Layout:** A orizzontale; B-C stretti derivati dallo stesso master shot; D
  primo piano; E splash inferiore con fascia nera per il lettering.
- **A:** Case apre la 92; Molly è seduta sulla testata di fondo e punta la
  fletcher. `SAFE_A` sulla parete alta.
- **B:** ritaglio su Case che chiude il fermo; Molly resta immobile alla stessa
  distanza. `SAFE_B` nel gutter superiore.
- **C:** ritaglio più stretto dello stesso master, terminale ancora tra i due;
  chiarimento su Linda e Wage. `SAFE_C` sulla parete destra.
- **D:** primo piano lungo lo stesso asse; Molly abbassa e ripone la fletcher,
  senza cambiare posto. `SAFE_D` nel gutter laterale.
- **E:** entrambe le mani aperte, dieci lame fuori; lenti e sorriso liberi. Le
  battute stanno nella fascia nera sopra il ritratto, non sull'arte.
- **Ritmo:** mira, verifica dell'arma, errore di Case, proposta, rivelazione.
